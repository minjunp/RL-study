...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
    ...
    while maxtol > tol:
        ...
        for i in range(nS):
            value_function[i] = 0
            for item in P[i][policy[i]]:
                value_function[i] += \
                    item[0] * (item[2] + gamma * prev_value_function[item[1]])

        maxtol = np.max(np.abs(value_function - prev_value_function))
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    for i in range(nS):
        ...
        for j in range(nA):
            for item in P[i][j]:
                Q[j] += item[0] * (item[2] + gamma * value_from_policy[item[1]])

        new_policy[i] = np.argmax(Q)
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
    ...
    while not np.array_equal(policy, new_policy):
        ...
        for i in range(nS):
            ...
            for j in range(nA):
                for item in P[i][j]:
                    Q[j] += item[0] * (item[2] + gamma * value_function[item[1]])
            new_policy[i] = np.argmax(Q)
            value_function[i] = np.max(Q)
