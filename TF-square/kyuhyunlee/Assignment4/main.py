...
class ClinicalDosingPolicy(StaticPolicy):
    def choose(self, x):
        ...
        dose = 4.0376 \
               ...
               - 0.6752 * x['Asian'] \
               + 0.4060 * x['Black'] \
               + 0.0443 * x['Unknown race'] \
               + 1.2799 * x['Carbamazepine (Tegretol)'] * x['Phenytoin (Dilantin)'] * x['Rifampin or Rifampicin'] \
...
class LinUCB(BanditPolicy):
    def __init__(self, n_arms, features, alpha=1.):
        ...
        self.A = [np.identity(len_features) for _ in range(self.n_arms)]
        self.b = [np.zeros([len_features, 1]) for _ in range(self.n_arms)]
        self.action_list = ('low', 'medium', 'high')
    ...
    def choose(self, x):
        ...
        x_val = np.array([x[item] for item in self.features]).reshape([-1, 1])
        
        p = np.zeros([self.n_arms])
        for arm in range(self.n_arms):
            theta = np.dot(np.linalg.inv(self.A[arm]), self.b[arm])
            p[arm] = np.dot(theta.transpose(), x_val) + \
                     self.alpha * np.sqrt(np.dot(np.dot(x_val.transpose(), np.linalg.inv(self.A[arm])), x_val))

        return self.action_list[np.argmax(p)]
    ...
    def update(self, x, a, r):
        ...
        x_val = np.array([x[item] for item in self.features]).reshape([-1, 1])
        arm_idx = self.action_list.index(a)

        self.A[arm_idx] += np.dot(x_val, x_val.transpose())
        self.b[arm_idx] += r * x_val
...
class eGreedyLinB(LinUCB):
    ...
    def choose(self, x):
        ...
        if np.random.uniform() > epsilon:
            ...
            return np.random.choice(self.action_list, p=np.exp(p)/np.sum(np.exp(p)))
...
class ThomSampB(BanditPolicy):
    def __init__(self, n_arms, features, alpha=1.):
        ...
        self.v2 = alpha 
        len_features = len(self.features)
        self.B = [np.identity(len_features) for _ in range(self.n_arms)]

        #Variable used to keep track of data needed to compute mu
        self.f = [np.zeros([len_features, 1]) for _ in range(self.n_arms)] 
    ...
    def choose(self, x):
        ...
        p = []
        for arm in range(self.n_arms):
            mu_mean = np.dot(np.linalg.inv(self.B[arm]), self.f[arm])
            mu_sample = np.random.multivariate_normal(mu_mean.squeeze(), np.linalg.inv(self.B[arm]))
            p.append(np.dot(self.f[arm].transpose(), mu_sample.reshape([-1, 1])))
    ...
    def update(self, x, a, r):
        ...
        for arm in range(self.n_arms):
            self.B[arm] += np.dot(self.f[arm], self.f[arm].transpose())
            if arm != arm_idx:
                #self.f[arm] += self.f[arm] * r
                self.f[arm] += x_val * r
