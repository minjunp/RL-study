	...
	def update_arms(self):
		...
		if self.update_arms_strategy == 'popular':
			...
			if self.num_arms > 1:	
				...
				for i in self.logs:
					arm_ids.append(i[1])
				for i in range(self.num_arms):
					arm_idx.append(arm_ids.count(i))
				a1 = np.argmax(arm_idx)
				arm_idx[a1] = 0
				a2 = np.argmax(arm_idx)
				
				self.arms[self.num_arms] = (self.arms[a1]+self.arms[a2])/2
				self.num_arms +=1
			else:
				return False
			#######################################################
		...
		if self.update_arms_strategy == 'corrective':
			...
			for log in self.logs:
				if log[1] != log[2]:
					incorrect.append(self.arms[log[2]])
			self.arms[self.num_arms] = np.mean(incorrect, axis = 0)
			self.num_arms +=1
			#######################################################
			#########          END YOUR CODE.          ############
		if self.update_arms_strategy == 'counterfactual':
			...
			grad = []
			for log in self.logs:
				user = self.users[log[0]]
				arm_true = self.arms[log[1]]
				grad.append((np.dot(theta.T, user) - np.dot(arm_true.T, user))*user)
			self.arms[self.num_arms] = theta + eta * np.sum(grad, axis = 0)
			self.num_arms +=1
			#######################################################
			#########          END YOUR CODE.          ############
