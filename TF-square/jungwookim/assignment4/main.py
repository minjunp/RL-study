...
class ClinicalDosingPolicy(StaticPolicy):
	def choose(self, x):
		...
		missing_or_mixed = x['Unknown race']
		enzyme_carba, enzyme_pheny, enzyme_rifap = x['Carbamazepine (Tegretol)'], x['Phenytoin (Dilantin)'], x['Rifampin or Rifampicin']
		enzyme_inducer_status = int(enzyme_carba)|int(enzyme_pheny)|int(enzyme_rifap)
		amiodarone_status = x['Amiodarone (Cordarone)']
		weekly_dose = 4.0376 - 0.2546 * age_in_decades + 0.0118 * height_in_cm + 0.0134*weight_in_kg - 0.6752*asian_race +0.4060*black_or_african_american + 0.0443*missing_or_mixed + 1.2799*enzyme_inducer_status - 0.5695*amiodarone_status
		return dose_class(weekly_dose**2)
		#######################################################
		######### 
...
class LinUCB(BanditPolicy):
	def __init__(self, n_arms, features, alpha=1.):
		...
		self.alpha = alpha
		self.A = np.array([np.identity(len(self.features))]*self.n_arms)
		self.b = np.array([np.zeros(len(self.features))] * self.n_arms)
	...
	def choose(self, x):
		...
		for i in range(self.n_arms):
			theta = np.dot(np.linalg.inv(self.A[i]) ,self.b[i])			
			p.append(np.dot(theta.T,x) + self.alpha*np.sqrt(np.dot(np.dot(x.T,np.linalg.inv(self.A[i])),x)))
		choose = np.argmax(p)
		action = {0:'low',1:'medium',2:'high'}
	...
	def update(self, x, a, r):
		...
		x = np.array([x[feature] for feature in self.features])
		arm = {'low':0, 'medium':1,'high':2}
		self.A[arm[a]] += np.outer(x,x.T)
		self.b[arm[a]] += r*x
		#######################################################
		#########          END YOUR CODE.          ############
...
class eGreedyLinB(LinUCB):
	...
	def choose(self, x):
		...
		if random_prob > epsilon:
			...
			return action[np.random.randint(0,self.n_arms-1)]
...
class ThomSampB(BanditPolicy):
	def __init__(self, n_arms, features, alpha=1.):
		...
		self.v2 = alpha 
		self.B = np.array([np.identity(len(self.features))]*self.n_arms)

		#Variable used to keep track of data needed to compute mu
		self.f = np.array([np.zeros(len(self.features))] * self.n_arms)
		#You can actually compute mu from B and f at each time step. So you don't have to use this.
		self.mu = [np.dot(np.linalg.inv(self.B[i]), self.f[i]) for i in range(self.n_arms)]
		#######################################################
		#########          END YOUR CODE.          ############
	...
	def choose(self, x):
		...
		p = []
		for i in range(self.n_arms):
			mu_sample = np.random.multivariate_normal(self.mu[i], self.v2 * np.linalg.inv(self.B[i]))
			p.append(np.dot(x.T,mu_sample))
		choose = np.argmax(p)
		return action[choose]
		#######################################################
		#########          END YOUR CODE.          ############
	...
	def update(self, x, a, r):
		...
		x = np.array([x[feature] for feature in self.features])
		arm = {'low':0, 'medium':1,'high':2}
		self.B[arm[a]] += np.outer(x,x.T)
		self.f[arm[a]] += r*x
		self.mu[arm[a]] = np.dot(np.linalg.inv(self.B[arm[a]]), self.f[arm[a]])
		#######################################################
		#########          END YOUR CODE.          ############
