...
class Linear(DQN):
    ...
    def initialize_models(self):
        ...
        self.q_network = nn.Linear(
            img_height * img_width * n_channels * self.config.state_history, num_actions
        )
    ...
    def get_q_values(self, state, network="q_network"):
        ...
        out = getattr(self, network)(state.flatten(1))
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def calc_loss(
        ...
        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############
        Q_samp = rewards + torch.where(
            done_mask,
            torch.tensor(0.0).to(done_mask.device),
            gamma * torch.max(target_q_values, 1)[0],
        )
        return F.mse_loss(q_values[torch.arange(len(q_values)), actions.long()], Q_samp)
        ##############################################################
        ######################## END YOUR CODE #######################
