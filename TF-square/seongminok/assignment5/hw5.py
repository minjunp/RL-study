...
class LinUCB:
	...
	def choose(self, x):
		...
		ucbs = [np.sqrt(np.linalg.multi_dot([x, A_inv, x])) for A_inv in A_invs]
		thetas = [np.dot(A_inv, b) for A_inv, b in zip(A_invs, self.b)]
		p = [np.dot(theta, x) + self.alpha * ucb for theta, ucb in zip(thetas, ucbs)]
		return np.argmax(p)
		#######################################################
		#########          END YOUR CODE.          ############
	...
	def add_arm_params(self):
		...
		#######################################################
		#########   YOUR CODE HERE - ~2 lines.   #############
		self.A.append(np.eye(self.num_features))
		self.b.append(np.zeros(self.num_features))
		#######################################################
		#########          END YOUR CODE.          ############
