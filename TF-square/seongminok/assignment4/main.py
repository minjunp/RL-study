...
class ClinicalDosingPolicy(StaticPolicy):
    def choose(self, x):
        ...
        out = 4.0376 \
            ...
            + 1.2799 * max(x['Carbamazepine (Tegretol)'], x['Phenytoin (Dilantin)'], x['Rifampin or Rifampicin']) \
...
class LinUCB(BanditPolicy):
    ...
    def choose(self, x):
        ...
        p = {act: np.dot(theta[act], x_feat) + self.alpha * np.linalg.multi_dot([x_feat, A_inv[act], x_feat]) for act in self.A}
...
class eGreedyLinB(LinUCB):
    ...
    def choose(self, x):
        ...
        x_feat =  np.array([x[feat] for feat in self.features])
        A_inv = {act: np.linalg.inv(self.A[act]) for act in self.A}
        p = {act: np.linalg.multi_dot([x_feat, A_inv[act], self.b[act]]) for act in self.A}
...
class ThomSampB(BanditPolicy):
    ...
    def choose(self, x):
        ...
        mu_sampled = {act: np.random.multivariate_normal(mu[act], self.v2 * B_inv[act]) for act in self.actions}
        expected_rewards = {act: np.dot(mu_sampled[act], x_feat) for act in self.actions}
