...
class PolicyGradient(object):
    ...
    def init_policy(self):
        ...
        net = build_mlp(self.observation_dim, self.action_dim, self.config.n_layers, self.config.layer_size)

        if self.discrete:
            self.policy = CategoricalPolicy(net)
        else:
            self.policy = GaussianPolicy(net, self.action_dim)
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            rows = np.arange(len(rewards)).reshape(1, -1).repeat(len(rewards), axis=0)
            columns = np.arange(len(rewards)).reshape(-1, 1).repeat(len(rewards), axis=1)
            diff = rows - columns
            discount_matrix = np.where(diff >= 0, np.exp(diff * np.log(self.config.gamma)), 0)
            returns = np.matmul(discount_matrix, rewards)
            #######################################################
            #########          END YOUR CODE.          ############
