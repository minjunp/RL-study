...
class BaselineNetwork(nn.Module):
    ...
    def __init__(self, env, config):
        ...
        self.network = build_mlp(env.observation_space.shape[0], 1, config.n_layers, config.layer_size)
    ...
    def update_baseline(self, returns, observations):
        ...
        for R, O in batch_iterator(returns, observations, shuffle=True):
            loss = nn.functional.mse_loss(self(O), R)
