...
class Simulator:
	...
	def update_arms(self):
		...
		if self.update_arms_strategy == 'popular':
			...
			counter = Counter([log[1] for log in self.logs])
			popular_arms = counter.most_common(2)
			if len(popular_arms) == 1:
				return False
			new_arm = np.sum([self.arms[arm[0]] for arm in popular_arms], axis=0) / len(popular_arms)
		...
		if self.update_arms_strategy == 'corrective':
			...
			incorrect_arms = [self.arms[log[2]] for log in self.logs if log[1] != log[2]]
			self.arms[self.num_arms] = np.mean(incorrect_arms, axis=0)
			self.num_arms += 1
			#######################################################
			#########          END YOUR CODE.          ############
		if self.update_arms_strategy == 'counterfactual':
			...
			theta, eta = np.zeros(self.num_features), 0.1
			users = np.stack([self.users[log[0]] for log in self.logs])
			arms = np.stack([self.arms[log[1]] for log in self.logs])
			coeffs = np.dot(users, theta) - np.sum(users * arms, axis=1)
			grad = np.dot(coeffs, users)
			theta_new = theta + eta * grad
			self.arms[self.num_arms] = theta_new
			self.num_arms += 1
			#######################################################
			#########          END YOUR CODE.          ############
