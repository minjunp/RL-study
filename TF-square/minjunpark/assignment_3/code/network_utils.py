...
def build_mlp(input_size, output_size, n_layers, size):
    ...
    layers = [nn.Linear(in_features=input_size, out_features=size)]
    for _ in range(n_layers):
        layers.extend([nn.Sequential(nn.Linear(in_features=size, out_features=size),
                            nn.ReLU())])
    layers.extend([nn.Linear(in_features=size, out_features=output_size)])
