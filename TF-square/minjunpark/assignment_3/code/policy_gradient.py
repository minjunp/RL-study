...
class PolicyGradient(object):
    ...
    def init_policy(self):
        ...
        BasePolicy = build_mlp(self.observation_dim, self.action_dim, self.config.n_layers, self.config.layer_size)
        print('Is discrete: ', self.discrete)
        if self.discrete:
            self.policy = CategoricalPolicy(BasePolicy)
        else:
            self.policy = GaussianPolicy(BasePolicy, self.action_dim)
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            gammas = np.power(self.config.gamma, range(len(rewards)))
            ...
            for i in range(len(rewards)):
                ...
                scaled_rewards = reward * gammas_t
