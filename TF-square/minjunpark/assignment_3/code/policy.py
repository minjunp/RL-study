...
class CategoricalPolicy(BasePolicy, nn.Module):
    ...
    def action_distribution(self, observations):
        ...
        # scores = self.network(observations).detach().numpy()
        # logits = softmax(scores)
        # distribution = torch.distributions.categorical.Categorical(torch.tensor(logits))
        #######################################################
        #########          END YOUR CODE.          ############
        return distribution
...
class GaussianPolicy(BasePolicy, nn.Module):
    def __init__(self, network, action_dim):
        ...
        # Need to be floating number
        self.log_std = nn.Parameter(torch.tensor([0.0]*action_dim))
        #######################################################
        #########          END YOUR CODE.          ############
    ...
    def std(self):
        ...
        #######################################################
        #########   YOUR CODE HERE - 1 line.       ############
        std = torch.tensor(np.exp(self.log_std.detach().numpy()))
        #######################################################
        #########          END YOUR CODE.          ############
        return std
    ...
    def action_distribution(self, observations):
        ...
        distribution = torch.distributions.multivariate_normal.MultivariateNormal(loc=self.network(observations), covariance_matrix=cov_mat)
        #######################################################
        #########          END YOUR CODE.          ############
        return distribution
