...
import code
from code import baseline_network
...
class TestBasic():
    ...
    def _test_sampled_actions(self):
        ...
        for action in actions:
            if discrete:
                ...
                print('cp-2')
                print(action_space.shape == action.shape)
    ...
    def _test_log_prob(self):
        ...
        print(torch.Size([len(observations)]))
        print(log_probs.shape == torch.Size([len(observations)]))
    ...
    def test_baseline_op(self):
        ...
        losses = []
        for i in range(1000):
            loss = float(self.baseline.update_baseline(returns, self.rand_obs).detach())
            losses.append(loss)
        ...
        # print('returns', returns)
        # print('self.rand_obs', self.rand_obs)
    ...
    def test_adv_basic(self):
        ...
        print(np.sum(res) == np.sum(returns))
...
## Passed:
# testbasic.test_cheetah_logprob()
# testbasic.test_pendulum_logprob()
# testbasic.test_get_returns_ones()

## Fixed - but not so sure
# testbasic.test_cartpole_logprob()

## Errors
testbasic.test_baseline_op() 
# testbasic.test_adv_basic()
