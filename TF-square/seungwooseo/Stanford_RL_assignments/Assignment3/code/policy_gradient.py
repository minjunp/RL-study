...
class PolicyGradient(object):
    ...
    def init_policy(self):
        ...
        if self.discrete:
            self.policy = CategoricalPolicy(network=network)
        else:
            self.policy = GaussianPolicy(network=network, action_dim=self.action_dim)
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            factors_mat = np.array([[self.config.gamma**(i-j) if i >= j else 0 for i in range(len(rewards))]
                                    for j in range(len(rewards))])   # factors at every t
            returns = np.matmul(factors_mat, np.array(rewards)[:, np.newaxis]).reshape((-1,))
            #######################################################
            #########          END YOUR CODE.          ############
    ...
    def update_policy(self, observations, actions, advantages):
        ...
        loss = -self.policy.action_distribution(observations).log_prob(actions) * advantages
