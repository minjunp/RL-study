...
def build_mlp(input_size, output_size, n_layers, size):
    ...
    net = nn.Sequential(nn.Linear(in_features=input_size, out_features=size), nn.ReLU())
    for i in range(n_layers):
        if i < n_layers-1:
            net.add_module(name=f"linear_{i}", module=nn.Linear(in_features=size, out_features=size))
            net.add_module(name=f"relu_{i}", module=nn.ReLU())
        else:
            net.add_module(name=f"linear_{i}", module=nn.Linear(in_features=size, out_features=output_size))
    return net
    #######################################################
    #########          END YOUR CODE.          ############
