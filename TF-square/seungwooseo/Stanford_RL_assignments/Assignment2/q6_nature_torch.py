...
class NatureQN(Linear):
    ...
    def initialize_models(self):
        ...
        self.q_network = nn.Sequential(nn.Conv2d(in_channels=channels, out_channels=32, kernel_size=(8, 8),
                                                 stride=4, padding=int(3 * img_height + 4) // 2),
                                       nn.ReLU(),
                                       nn.Conv2d(in_channels=32, out_channels=64, kernel_size=(4, 4),
                                                 stride=2, padding=int(img_height + 2) // 2),
                                       nn.ReLU(),
                                       nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(3, 3),
                                                 stride=1, padding=1),
                                       nn.ReLU(),
                                       nn.Flatten(),
                                       nn.Linear(in_features=int(img_height * img_width * 64),
                                                 out_features=512),
                                       nn.ReLU(),
                                       nn.Linear(in_features=512, out_features=num_actions)
                                       )
        self.target_network = nn.Sequential(nn.Conv2d(in_channels=channels, out_channels=32, kernel_size=(8, 8),
                                                      stride=4, padding=int(3 * img_height + 4) // 2),
                                            nn.ReLU(),
                                            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=(4, 4),
                                                      stride=2, padding=int(img_height + 2) // 2),
    ...
    def get_q_values(self, state, network):
        ...
        trans_state = state.permute((0, 3, 2, 1))
        out = self.q_network(trans_state) if network == 'q_network' else self.target_network(trans_state)
        ##############################################################
        ######################## END YOUR CODE #######################
        return out
